class Game
  ...
  def game_turn(which_player)
    ...
    @display.throw(self)
    puts ''
    puts "#{which_player.name} on wich boardcase do you wanna play?"
    puts "Choose one letter for the line and choose one number for column. exemple : B2 for the center boardcase"
  ...
  def enter_input(which_player)
    ...
    if @current_board.boardcase_array.any? {|boardcase| boardcase.coords == player_choice && boardcase.status == nil}
      ...
      puts "Wrong boardcase name or is allready taken"
      enter_input(which_player)
    end
  end
